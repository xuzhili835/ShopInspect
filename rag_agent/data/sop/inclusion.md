# 夹杂物(inclusion)缺陷处置 SOP

## 1. 定义与外观特征
**类别溯源**:本类命名与样本定义出自 NEU-DET 数据集原始文献(Song & Yan, Applied Surface Science, 2013, DOI: 10.1016/j.apsusc.2013.09.002),原文对本类的描述为「轧入带钢表面的非金属夹杂物,呈点/条状暗斑(non-metallic inclusion rolled into surface)」(意译);GB/T 14977 仅提供分级判定总纲、不含逐类命名,六类与各参考资料的对应关系见《00_总则》第 6 节对照表。

非金属夹杂物暴露于带钢表面形成的点状、短线状或条状暗色斑,常沿轧向断续排布,周边可伴脱落凹坑;金相与能谱分析可确认夹杂类型与来源(脱氧产物/耐材/渣类),是判别成因与责任工序的关键。

## 2. 成因机理
炼钢脱氧产物、中间包与结晶器耐材剥落、保护浇铸不良卷入的夹渣,随铸坯进入轧制后被延展拉长,暴露于成品表面;宝钢 1580 线研究将异物压入归纳为清坯毛刺掉落、锈蚀物脱落轧入等多类来源,均指向上游工序洁净度。

## 3. 检测与评级
目视 + 金相/能谱定性;按数量、尺寸、分布密度对照表面等级评级(界限按企业标准执行);同炉号多卷出现即触发工艺追溯。

## 4. 分级处置
夹杂无法靠修磨根治(去除表层后内层仍可能再现),以评级判级为主:
- **单个细小、数量在等级内**:按相应表面等级放行或降一级交付。
- **密集分布或条状超长**:该段【判废】。
- **同炉号批量出现**:该浇次【停产】追溯,未查明原因前暂缓同工艺批次交付。

## 5. 复检验收
按炉批抽查复评,表面等级满足订单要求;金相定性记录存档;整改后首件加严检验。

## 6. 根因反馈与预防
反馈炼钢工序:脱氧制度、中包/结晶器耐材寿命管理、保护浇铸氩封状态;区分耐材类与脱氧类夹杂对症整改;建立炉号-浇次-缺陷对应台账。

## 参考文献
1. GB/T 14977-2008 热轧钢板表面质量的一般要求[S]. 标准详情 · 全国标准信息公共服务平台: https://std.samr.gov.cn/gb/search/gbDetailed?id=71F772D78682D3A7E05397BE0A0AB82A
2. 热轧带钢异物压入缺陷分析及改进措施[J]. 金属世界, 2021(1). DOI: 10.3969/j.issn.1000-6826.2021.01.0013. 期刊官网全文: https://metalworld.ustb.edu.cn/cn/article/pdf/preview/10.3969/j.issn.1000-6826.2021.01.0013.pdf(异物来源分类,宝钢 1580mm 产线)
3. Song K, Yan Y. A noise robust method based on completed local binary patterns for hot-rolled steel strip surface defects[J]. Applied Surface Science, 2013, 285(B): 858-864. DOI: 10.1016/j.apsusc.2013.09.002: https://doi.org/10.1016/j.apsusc.2013.09.002(NEU-DET 夹杂类样本定义)
